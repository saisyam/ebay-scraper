...
def get_product(url, count):
    for i in range(1, count):
        ...
        #pagination_div = section.find("div",{'class':'b-pagination'})
        #pages = pagination_div.find("ol", {'class':'ebayui-pagination__ol'}).find_all('li')
        for item in items:
            purl = item.find("div", {'class':'s-item__image'}).find("a")['href']
            yield scrape_product(purl)
...
def scrape_product(purl):
    ...
    if cpanel is not None:
        return scrape_product_type_2(soup, purl)
    elif container is not None:
        return scrape_product_type_3(soup, purl)
    else:
        return scrape_product_type_1(soup, purl)
...
def scrape_product_type_1(soup, url):
    ...
    for img in imgs:
        ...
        img_urls.append(img.find('img')['src'])
...
if len(sys.argv) == 4:
    ...
    print("Usage: python3 ebay_scraper.py <url> <pagecount> <outfile>")
    exit()
